    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
            <button onClick={() => setIsAdding(!isAdding)} className="w-full md:w-auto bg-black text-white px-5 py-2.5 rounded-lg font-bold uppercase tracking-wide text-xs hover:bg-gray-800 transition-all flex items-center justify-center gap-2">
                {isAdding ? <><X size={16}/> Fechar</> : <><Plus size={16}/> Novo Projeto</>}
            </button>
        </div>
      </div>
      ...
      <div className="relative">
          ...
          <input type="text" placeholder="Pesquisar projetos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm font-bold focus:outline-none shadow-sm" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredProjects.map(project => (
            <div key={project.id} onClick={() => setEditingProject(project)} className="group bg-white rounded-xl p-3 shadow-apple hover:shadow-float transition-all border cursor-pointer relative overflow-hidden flex flex-col gap-2.5 animate-in fade-in" style={{ borderColor: project.brandColor ? `${project.brandColor}30` : '#e5e7eb' }}>
